...
const MobileSideBar = ({ isOpen, onClose }: MobileSideBarProps) => {
    ...
    const MenuItem = ({
        ...
        return (
          <Link
            ...
            } hover:bg-gray-700 menu-title text-sm font-semibold text-gray-700 menu-item-active:text-primary menu-link-hover:!text-primary`}
          >
            <i className={`mr-2 ${icon}`}></i>
            <span onClick={onClick} className="text-sm font-semibold px-2">{title}</span>
          </Link>
        );
      };
  ...
  return (
    ...
    >
      <div ref={sidebarRef} className="w-64 bg-white h-full shadow-lg">
